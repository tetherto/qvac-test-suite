...
const createToolsTest = (
  ...
  expectation: { validation: 'type'; expectedType: 'string' | 'number' | 'array' | 'embedding' } = {
    validation: 'type',
    expectedType: 'string',
  }
): TestDefinition => ({
  ...
  expectation,
  metadata: { category: 'tools', dependency: 'tools', estimatedDurationMs: 15000 },
});
...
export const toolsTests = [toolsSimpleFunction, toolsMultipleFunctions, ...additionalToolsTests];
