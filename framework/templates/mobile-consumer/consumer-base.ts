...
export abstract class ConsumerBase {
  ...
  protected getRequiredModelType(
    testId: string
    ...
    if (testId.startsWith('transcription') || testId.startsWith('config-reload')) {
      ...
      if (testId.includes('backward-compatibility') || testId.includes('detection')) {
  ...
  protected getTestTimeout(testId: string): number {
    const isDestructiveTest =
      testId.includes('embed-python') ||
      testId.includes('embed-javascript') ||
      testId.includes('embed-json') ||
      testId.includes('embed-html') ||
      testId.includes('very-long') ||
      testId.includes('extremely-long') ||
    ...
    if (isDestructiveTest) {
      ...
      const isLongTts =
        testId.includes('stack-overflow') ||
        testId.includes('very-long') ||
        testId.includes('extremely-long') ||
        testId.includes('large-buffer');
      if (isLongTts) {
