...
export async function runLocalAndroid(opts: AndroidOptions) {
  try {
    ...
    const deviceLog = startDeviceLogCapture(reportDir, 'android', { serial, packageName: androidPackage });
    ...
    if (deviceLog) pidEntries.push({ name: 'device-log', pid: deviceLog.child.pid ?? 0, logPath: deviceLog.logPath });
  ...
  if (fs.existsSync(derivedData)) {
    ...
    for (const dir of dirs) {
      ...
      for (const config of fs.readdirSync(productsDir).filter((d) => d.endsWith('-iphoneos'))) {
        const apps = fs.readdirSync(path.join(productsDir, config)).filter((f) => f.endsWith('.app'));
        if (apps.length > 0) return path.join(productsDir, config, apps[0]);
      }
    }
  }

  throw new Error(`Could not find built .app for scheme "${scheme}". Check that the build succeeded.`);
}
