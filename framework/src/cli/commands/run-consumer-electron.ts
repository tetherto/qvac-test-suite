...
function findFile(root: string, predicate: (filePath: string) => boolean, maxDepth: number = 5): string | undefined {
...
function resolveLaunchCommand(executable: string, platform: NodeJS.Platform, env: NodeJS.ProcessEnv) {
...
export async function runConsumerElectron(options: ConsumerElectronOptions) {
  try {
    ...
    const executable = resolvePackagedExecutable(appDir, electron.outDir ?? 'out', appName, platform, arch);
    ...
    if (launch.command === 'xvfb-run') {
      ...
      console.log('🖥️  No DISPLAY or xvfb-run detected; launching Electron with headless Ozone flags');
    }
