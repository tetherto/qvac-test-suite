...
function copyTemplateFiles(
  ...
  const files = ['App.tsx', 'batch-consumer.tsx', 'consumer-wrapper.tsx', 'metro.config.js', 'babel.config.js'];
...
function generateAppJson(
  ...
  const networkSecurityPluginIndex = appConfig.expo.plugins.findIndex(
    (p: string | [string, unknown]) => typeof p === 'string' && p.includes('withNetworkSecurityConfig')
  );

  if (networkSecurityPluginIndex !== -1 && caCert) {
    // Replace string plugin reference with array [plugin, caCert]
    appConfig.expo.plugins[networkSecurityPluginIndex] = ['./plugins/withNetworkSecurityConfig.js', caCert];
  ...
  if (userPlugins && userPlugins.length > 0) {
    const assetPluginIndex = appConfig.expo.plugins.findIndex((p: string | [string, unknown]) => p === 'expo-asset');
...
async function generateAssetDeclarations(configDir: string, outputDir: string, assetPatterns: string[]): Promise<void> {
  ...
  const lines = ['// Auto-generated asset declarations - DO NOT EDIT', '// Generated during mobile consumer build', ''];
