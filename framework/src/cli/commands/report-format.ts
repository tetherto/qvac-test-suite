...
function generateMarkdown(comparison: {
  ...
  // Summary
  const baselineRateNum =
    (comparison.summary.baseline.passed / comparison.summary.baseline.total) * 100
  const currentRateNum =
    (comparison.summary.current.passed / comparison.summary.current.total) * 100
