...
function runArgs(packageManager: string, script: string, platform: NodeJS.Platform, arch: string): string[] {
...
function runPackageManager(packageManager: string, args: string[], cwd: string, env: NodeJS.ProcessEnv): void {
...
export async function buildConsumerElectron(options: BuildConsumerElectronOptions) {
  ...
  runPackageManager(packageManager, runArgs(packageManager, packageScript, platform, arch), appDir, buildEnv);
