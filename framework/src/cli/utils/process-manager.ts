 ...
 */
export function spawnTracked(command: string, args: string[], opts: SpawnTrackedOptions): TrackedProcess {
 ...
 */
export function printPidTable(entries: Array<{ name: string; pid: number; logPath: string }>): void {
  ...
  for (const entry of entries) {
    console.log(`  ${entry.name.padEnd(nameWidth)}  ${String(entry.pid).padEnd(pidWidth)}  ${entry.logPath}`);
  }
  console.log('');
}
...
export function createReportDir(configDir: string, runId: string, overrideDir?: string): string {
  const reportDir = overrideDir ? path.resolve(overrideDir) : path.resolve(configDir, 'reports', `local-${runId}`);
