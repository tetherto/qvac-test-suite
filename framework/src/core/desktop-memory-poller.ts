 ...
 * just like the mobile in-app poller does.
 *
 ...
 *   QVAC_DESKTOP_MEM_INTERVAL_MS  — tick period in ms
 *   QVAC_DESKTOP_MEM_DISABLED=1   — turn the poller off entirely
 */

export interface DesktopMemoryPollerOptions {
  client: MqttClient;
  runId: string;
  consumerId: string;
  intervalMs?: number;
...
export interface DesktopMemoryPollerHandle {
  stop: () => void;
}

const DEFAULT_INTERVAL_MS = 200;

export function startDesktopMemoryPoller(opts: DesktopMemoryPollerOptions): DesktopMemoryPollerHandle | null {
  if (process.env.QVAC_DESKTOP_MEM_DISABLED === '1') return null;

  const intervalMs = opts.intervalMs ?? Number(process.env.QVAC_DESKTOP_MEM_INTERVAL_MS || DEFAULT_INTERVAL_MS);
