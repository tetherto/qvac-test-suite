...
const typeExpectationSchema = z.object({
    ...
    expectedType: z.enum(['string', 'number', 'array', 'embedding'])
        .describe('Expected JavaScript type or special type (embedding = array of numbers)'),
    minDimensions: z.number().optional()
        .describe('For embedding type: minimum array length')
});

const errorExpectationSchema = z.object({
    validation: z.literal('throws-error'),
    errorContains: z.string()
        .describe('String that must be present in the error message')
});
...
export const testDefinitionSchema = z.object({
    ...
    params: z.any()
        .describe('Parameters to pass to the test executor'),

    expectation: expectationSchema
        .describe('Expected outcome specification for validation'),
