...
const consumerPlatformSchema = z.object({
    ...
    entry: z.string()
        .describe('Entry point file for the consumer (e.g., ./tests/desktop/consumer.ts)'),
...
export const qvacTestConfigSchema = z.object({
    ...
    sourceRepo: z.string().optional()
        .describe('Path to source repository being tested (for remote testing, e.g., "../sdk-repo"). If omitted, uses ./src from current directory'),

    testDir: z.string()
        .describe('Directory containing test definitions (e.g., "./tests")'),

    runIdStrategy: z.union([
        z.literal('auto'),
        z.literal('manual'),
        z.function()
    ]).optional()
        .default('auto')
        .describe('Run ID generation: "auto" generates repo-branch-commit-timestamp, "manual" requires --runId flag, or custom function'),

    consumers: z.object({
        desktop: consumerPlatformSchema.optional()
            .describe('Desktop consumer configuration for Node.js platforms'),

        mobile: consumerPlatformSchema.optional()
            .describe('Mobile consumer configuration for React Native platforms')
