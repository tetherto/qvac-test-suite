...
async function testEmbedding() {
	try {
		...
		// Step 2: Test empty text (should work based on test results)
		console.log("\n2️⃣  Testing empty text...");
		const startEmpty = Date.now();
		const emptyResult = await Promise.race([
			runEmbed({ modelId, text: "" }),
			new Promise((_, reject) =>
				setTimeout(() => reject(new Error("Timeout after 5s")), 5000)
			),
		]);
		const emptyDuration = Date.now() - startEmpty;
		console.log(`✅ Empty text completed in ${emptyDuration}ms`);
		console.log(`   Result: ${JSON.stringify(emptyResult).substring(0, 100)}...`);

		// Step 3: Test simple text (this should hang based on test results)
		console.log("\n3️⃣  Testing simple text: 'Hello'...");
		...
		// Step 4: Test very short text
		console.log("\n4️⃣  Testing very short text: 'Hi'...");
		console.log("   ⏳ Waiting max 10 seconds...");
